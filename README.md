# Muhammad Saad Jamal - Personal Portfolio

> A clean, responsive, and technical personal portfolio website showcasing production-grade Flutter applications, clean architecture projects, and mobile development case studies.

## ⚡ Features
*   **Dark-Mode First Design:** A minimalist, technical visual identity utilizing Space Grotesk and IBM Plex Mono typography.
*   **Project Showcases:** High-fidelity multi-screen device mockups detailing engineering decisions, tech stacks, and architecture for six major projects.
*   **Direct App Downloads:** Hosted APK downloads for select production-ready applications (Drips Water, Task Flow).
*   **Fully Responsive:** Fluid layouts built with modern CSS Flexbox/Grid that scale perfectly from desktop down to mobile.
*   **Integrated Contact:** Working contact form configured via Web3Forms, alongside vector social links.

## 🛠️ Featured Projects
This portfolio highlights comprehensive case studies for the following applications:
1.  **Drips Water:** Water Delivery & Logistics Ecosystem featuring a role-based Admin Dashboard and a 24/7 Dialogflow AI chatbot.
2.  **Doctor Hunt:** Healthcare Booking Ecosystem for real-time doctor discovery and pharmacy integration.
3.  **Task Flow:** Real-Time Productivity System with cloud sync, CRUD operations, and local notifications.
4.  **Eventify:** Event Registration System with Firebase Auth, Hive bookmarking, and QR ticket generation.
5.  **Glitz Mart:** E-Commerce platform integrated with RESTful APIs, category navigation, and cart management.
6.  **Grade Wise:** Offline-first GPA/CGPA Academic Calculator with semester management.

## 💻 Tech Stack
*   **HTML5:** Semantic HTML structuring for optimal accessibility and SEO.
*   **CSS3:** Custom CSS variables for theme management, responsive media queries, and smooth transitions.
*   **JavaScript:** Lightweight vanilla JS for mobile navigation toggling and interactive UI elements.
*   **Web3Forms:** Serverless form submission handling.

## 📬 Connect
Email: saadj4775@gmail.com

LinkedIn: [in/muhammadsaadjamal](https://www.linkedin.com/in/muhammadsaadjamal/)

GitHub: @DevExplorerr

Designed and built by Muhammad Saad Jamal.
